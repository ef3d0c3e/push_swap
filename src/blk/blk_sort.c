...
static void sort_3(t_state *s, t_blk *blk)
{
	...
	if ((blk->dest & __BLK_POS) == __BLK_TOP)
	{
		ft_printf("(%d, %d, %d, %d)\n", u, v, w, blk->dest);
		((void)((u > v && v > w
				&& (encode(s, __STACK_OP_ROTATE, blk), encode(s, __STACK_OP_SWAP, blk), 1))
			|| (u > w && w > v
				&& (encode(s, __STACK_OP_ROTATE, blk), 1))
			|| (v > u && u > w
				&& (encode(s, __STACK_OP_REV_ROTATE, blk), 1))
			|| (v > w && w > u
				&& (encode(s, __STACK_OP_SWAP, blk), encode(s, __STACK_OP_ROTATE, blk), 1))
			|| (w > u && u > v
...
static inline void sort_3_orig(t_state *s, t_blk *blk)
{
	const int	u = blk_value(s, blk, 0);
	const int	v = blk_value(s, blk, 1);
	const int	w = blk_value(s, blk, 2);
	...
	if (blk->dest == BLK_A_TOP)
		...
		((void)(
		   (u > v && v > w && (op(s, STACK_OP_RRB), op(s, STACK_OP_RRB), op(s, STACK_OP_RRB), op(s, STACK_OP_PA), op(s, STACK_OP_PA), op(s, STACK_OP_PA), 1))
